...
def ocr09(imagepath):
    ...
    # 识别审批表
        ...
        ret, binary = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        cv2.imwrite('binary8.jpg', binary)
        # 膨胀
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
        imgerode = cv2.erode(binary, kernel)
        '''
        string1 = pytesseract.image_to_string(cropped, lang='chi_sim', config='--psm 7 --oem 2')
        string1 = string1.replace(' ', '').replace('|', '').replace('“', '').replace('_', '').replace('\n', '')
        # print(string)
...
def wholeEXE(renlist0, pdfdoc0):
    record = []
    record.append("REN ")
    oldname = pdfdoc0.name
    record.append(oldname)
    if index == '1':
        # 封面
        jpgpath1 = pdf2jpg(pdfdoc0, 0)
        record.append(ocr09(jpgpath1))
        renlist0.append(record)
    else:
        # 封面
        jpgpath1 = pdf2jpg(pdfdoc0, 0)
        record.append(ocr09(jpgpath1))
        # 审批表
        huzhunum = (pdfdoc0.page_count - 7) // 2
        jpgpath8 = pdf2jpg(pdfdoc0, huzhunum + 6)
    ...
    for f in os.listdir(mainPath):
        if f.endswith('.pdf'):
            ...
            if pages > 8:
                pdflist.append(pdf)
    threads = []
    for pdfdoc in pdflist:
        recv_thread = threading.Thread(target=wholeEXE, args=(renlist, pdfdoc))
